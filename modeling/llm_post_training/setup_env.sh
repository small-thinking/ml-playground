...
echo ""
echo "   Multi-GPU (if available):"
echo "   - GRPO: torchrun --nproc_per_node=\$(nvidia-smi -L | wc -l) reasoning_grpo.py --model-size 3B --use-lora"
echo "   - SFT:  python instruction_sft.py --model-size 3B --use-lora  # (uses device_map='auto')"
echo ""
echo "   Check GPU availability: python -c 'import torch; print(f\"GPUs: {torch.cuda.device_count()}\")'"
