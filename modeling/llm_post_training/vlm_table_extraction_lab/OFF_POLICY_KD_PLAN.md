...
2026-09-14。状态：方案供审阅，尚未生成 teacher rollout、训练 KD 或创建 W&B run。
...
流程：图片 → teacher 生成 HTML → 冻结原始 token 轨迹 → teacher Top20 概率 → student 学习 → 完整 Dev/Test。

## Teacher 与 student

首选 teacher：`Qwen/Qwen3.5-9B`，关闭 thinking。它与当前 student 同系列，
费用适合验证链路；目前没有测量它在 RD 上的抽取质量，不能假定更大就一定更好。
不选旧的 Qwen3.5-27B/35B-A3B：当前官方名单已将这两个 instruct 型号列为 retired。
9B 若不足以提供有价值的目标，再考虑 397B-A17B；它不属于本轮默认执行范围。
...
- Teacher tokenizer/processor revision：`c202236235762e1c871ad0ccb60c8ee5ba337b9a`。
- Student tokenizer/processor revision：`851bf6e806efd8d0a36b00ddf55e13ccb7b8cd0a`。
- 两侧 `tokenizer.json` 字节 SHA-256 均为
...
- 账户的只读 server capabilities 已确认 4B、9B、397B-A17B 均可见；未调用采样或训练。
...
- 既有 `load_renderer` 目前只允许 4B；实现时需显式增加经过验证的 9B 与其 revision，
...
| Soft targets | Top20，记录未归一化 logprobs 与原始 probability mass，再在 Top20 内归一化 |
...
W&B 分组应标记实验/数据协议，不能把 9B teacher 混入 4B student 的同模型提升曲线。
...
| Teacher 第二次 forward 取 Top20 | $0.085 | $0.137 | $0.847 | $1.375 |
...
缓存使用 assistant positions 的 int32 IDs + float32 logprobs：80×1k×20 约
12.8 MB；800×1k×20 约 128 MB，另加索引/metadata。JSON 嵌套数组会大得多。
这些数组及原始 token IDs 不上传 Git/W&B。

时间尚不能精确预测。参考：上一轮 4B 的 100 optimizer steps 用时 8.42 分钟，
完整 Test100 NLL+生成用时 5.36 分钟；9B 两次前向、Top-K 传输和队列延迟尚未测量。
...
待确认 Top-K 近似符合本次“传统 KD”的目标后，按以上合同实现并运行获准的小烟测。
