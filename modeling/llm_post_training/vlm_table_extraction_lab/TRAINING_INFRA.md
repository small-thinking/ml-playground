...
核实日期：2026-09-13。用户已选择首轮使用 Tinker；以后保留迁移到自租 GPU
与 TRL 的空间。当前只完成数据和离线 Judge 准备，未调用付费模型或启动训练。

## 当前路线

Tinker + Qwen3.5-4B + LoRA + W&B。先跑 20 条 Dev 验证抽取链路，建立
100 条 Dev baseline，再用嵌套训练子集中的 80 条做最多 2 epochs SFT，
